...
import saveUserTokens from "./save-user-token";
import { Tokens } from "./save-user-token";

// Define the backend URL and the maximum time for token refresh
const MAX_TIME_REFRESH =
  60 *
  1000; // Use this to determine when to refresh tokens
...
function makeFetch(
  ...
  return async function (
    ...
    return fetch(
      `${process.env.NEXT_PUBLIC_FAKESTORE_API}${path}`,
      {
        headers:
          {
